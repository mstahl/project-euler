...
func numericize(word string) (n int) {
  for _, v := range word {
    n += int(v - 'A') + 1
  }
  return
...
func main() {
  triangle_numbers := make(map[int]bool)
  for i, triangle := 1, 0; triangle < 200; i++ {
    triangle += i
    triangle_numbers[triangle] = true
  }
  words := []string{
