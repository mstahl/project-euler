...
func product(array []int) (product int) {
  product = 1
  for _, x := range array {
    product *= x
  }
...
func main() {
  ...
  for _, row := range grid {
    for i := 0; i < len(row) - 4; i++ {
      if p := product(row[i:i + 4]); greatest < p {
        greatest = p
      }
    }
  }
  ...
  // Then check diagonals going up and to the right
  for row := 3; row < len(grid); row++ {
    for col := 0; col < len(grid[0]) - 4; col++ {
      if p := product([]int{
        grid[row][col],
        grid[row - 1][col + 1],
        grid[row - 2][col + 2],
  ...
  // And finally diagonals going down and to the right
  for row := 0; row < len(grid) - 4; row++ {
    for col := 0; col < len(grid[0]) - 4; col++ {
      if p := product([]int{
        grid[row][col],
