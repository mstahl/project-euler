...
const uint64_t big_huge_number[1000] = {
...
#define SEQUENCE_LENGTH 13

int main(void) {
  uint64_t sequence;
  uint64_t best_sequence = 0;

  for(int i = 0; i < 1000 - SEQUENCE_LENGTH; ++i) {
    sequence = 1;
    for(int j = 0; j < SEQUENCE_LENGTH; ++j) {
      sequence *= big_huge_number[i + j];
    }
    if(best_sequence < sequence) best_sequence = sequence;
  }
  printf("%lu\n", best_sequence);
}
