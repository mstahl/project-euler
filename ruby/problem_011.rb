...
max = 0
best = []
[grid, grid.transpose, rotate_right(grid).transpose, rotate_left(grid).transpose].each do |g|
  g.each do |row|
    each_cons(row) do |group|
      if group.first(4).inject(1) {|s, n| s * n} > max then
        max = group.first(4).inject(1) {|s, n| s * n}
        best = group.first(4)
      end
    end
  end
