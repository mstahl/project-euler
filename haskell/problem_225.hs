...
tribonaccis_mod_n n = 
  tribonaccis 
  where 
  tribonaccis = 1 : 1 : 3 : zipWith3 (\a b c -> (a + b + c) `mod` n)
                                     tribonaccis
                                     (drop 1 tribonaccis)
                                     (drop 2 tribonaccis)
...
-- test [Integer]
-- 
-- Examines the tribonacci list to see if it repeats itself. The first '1' is
-- deliberately left off the tribonaccis_mod_n output above.
test (0:_) = False
test (1:1:1:xs) = True
test (_:xs) = test xs
