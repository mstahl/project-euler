...
-- 
-- If (n_(1),n_(2),n_(3)) indicates a Nim position consisting of heaps of size
-- n_(1), n_(2) and n_(3) then there is a simple function X(n_(1),n_(2),n_(3))
...
main :: IO ()
main = do print $ length $ [n | n <- [1..(2^30)]
                              , nim_sum [n, 2 * n, 3 * n] == 0
                              ]

-- This is a fairly well-known function in combinatorial game theory, and is
-- called the Nim sum function. It indicates only whether the current game
-- described can be won, not what the best move is (though the best move will
-- always be one that returns the game to a Nim sum of zero).
